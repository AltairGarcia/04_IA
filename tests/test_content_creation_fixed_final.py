...
def test_generate_twitter_thread_success(mock_chat_google_genai, content_creator_instance, tmp_path):
    ...
    expected_filename_start = "twitter_thread_test_twitter_topic"
    output_files = os.listdir(tmp_path)
    assert any(f.startswith(expected_filename_start) and f.endswith(".md") for f in output_files), f"Files found: {output_files}"
