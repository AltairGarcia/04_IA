...
def test_generate_blog_post_success(mock_chat_google_genai, content_creator_instance, tmp_path):
    ...
    assert "200 words" in called_prompt # target_word_count (minimum enforced by function)
...
def test_generate_blog_post_llm_api_error(mock_chat_google_genai, content_creator_instance):
    ...
    assert "error" in result
    assert "An unexpected error occurred during blog post generation: Simulated API Error" in result["error"]
    assert "filepath" not in result
...
def test_generate_blog_post_file_write_error(mock_chat_google_genai, mock_file_open, mock_makedirs, content_creator_instance, tmp_path):
    ...
    assert "error" in result
    assert "An unexpected error occurred during blog post generation: Simulated File Write Error" in result["error"]
    # When file write fails, no filepath is returned in the error case
...
EXPECTED_TWITTER_THREAD_TWEETS = ["1/ Test tweet 1.", "2/ Test tweet 2 with #TestHashtag."]
MOCK_GEMINI_TWITTER_RESPONSE_SUCCESS = {
    "title": "Test Twitter Thread",
    "tweets": EXPECTED_TWITTER_THREAD_TWEETS
}
...
def test_generate_twitter_thread_success(mock_chat_google_genai, content_creator_instance, tmp_path):
    ...
    # Remove platform parameter as it's not accepted by the function
    ...
    # The actual function creates files with timestamp, not topic name
    expected_filename_start = "twitter_thread_"
    output_files = os.listdir(tmp_path)
    assert any(f.startswith(expected_filename_start) and f.endswith(".txt") for f in output_files), f"Files found: {output_files}"
...
def test_generate_twitter_thread_json_decode_error(mock_chat_google_genai, content_creator_instance):
    ...
    assert "error" in result
    assert "Could not parse Twitter thread from AI response" in result["error"]

@patch('langchain_google_genai.ChatGoogleGenerativeAI')
def test_generate_twitter_thread_missing_keys_in_json(mock_chat_google_genai, content_creator_instance):
    """Test Twitter thread generation with missing required keys in JSON."""
    mock_llm_instance = MagicMock()
    json_missing_keys = {"some_other_data": "value"} # Missing required keys
    mock_llm_instance.invoke.return_value = MagicMock(content=json.dumps(json_missing_keys))
    mock_chat_google_genai.return_value = mock_llm_instance
...
def test_generate_twitter_thread_empty_topic(mock_chat_google_genai, content_creator_instance):
    ...
    assert "error" in result and "Topic or base content must be provided." in result["error"]
    assert result.get("filepath") == ""
...
def test_generate_twitter_thread_file_write_error(mock_chat_google_genai, mock_file_open, mock_makedirs, content_creator_instance, tmp_path):
    ...
    # Twitter thread function handles file write errors gracefully - no error returned
    assert "error" not in result  # Twitter thread doesn't return error for file write failures
    assert "filepath" in result and result["filepath"] == ""  # Empty filepath indicates write failure
