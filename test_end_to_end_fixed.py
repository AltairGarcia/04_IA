...
class EndToEndTester:
    ...
    def _ensure_health_server_running(self):
        """Ensure health server is running on port 8502."""
        try:
            # Try to connect to health server
            health_url = "http://localhost:8502"
            response = requests.get(f"{health_url}/health", timeout=2)
            if response.status_code == 200:
                return True
        except:
            # Health server not running, try to start it
            try:
                import sys
                import subprocess
                import time
                ...
                # Start health server in background using full path
                subprocess.Popen([
                    python_path, "-c", 
                    "from health_server import start_health_server; start_health_server(port=8502, debug=False)"
    ...
    def test_health_monitoring_system(self) -> bool:
        ...
        start_time = time.time()
        try:
            # Ensure health server is running before testing
            self._ensure_health_server_running()
            
            # Test health check endpoint - use health server on port 8502
            health_url = self.base_url.replace(':8501', ':8502')
